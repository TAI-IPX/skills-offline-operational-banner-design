...
def _wide_a5b_alpha_threshold() -> float:
    """3320×500 A5b BiRefNet 条带 alpha 二值化阈值，默认 0.6；可用环境变量 WIDE_A5B_ALPHA_THRESHOLD=0.45 等调软边缘。"""
...
def _moxingpt_edit_image(
    ...
    *,
    ...
    """
    MoxinGPT 图编：通过 moxin.studio /v1/chat/completions 编辑图片。
    ...
    try:
        ...
        pass
    print(f"[moxingpt edit] {orig_w or '?'}×{orig_h or '?'} -> 编辑中...", flush=True)
    ...
    if not img_url and not b64_out:
        raise RuntimeError(f"moxingpt edit no image. resp: " + _json.dumps(data, ensure_ascii=False)[:500])
    ...
    if not keep_returned_size and orig_w and orig_h:
        try:
            ...
            if ed.size != (orig_w, orig_h):
                ...
                print(f"[moxingpt edit] 已缩放回 {orig_w}×{orig_h}", flush=True)
        except Exception:
            pass
...
def wide_from_fill(fill_image_path: str, output_path: str, bbox_file: str | None = None) -> None:
    """
    复用已有填充图：将 fill_image_path（如 2048×512 的 tianchong.png）按 cover 缩放至 3320×500，
    再做 A5b 顶部条带，写入 output_path。供 run_all_presets 在 Step 1b 只做一次填充时使用。
    
    bbox_file: 可选，主体 bbox 文件路径（shared_subject_bbox.txt），格式为 x_min,y_min,x_max,y_max（归一化坐标 0-1）。
               若提供，则智能对齐主体 bbox 中心到安全区中心；否则回退到居中裁切。
    """
    from PIL import Image
    ...
    # 智能对齐：读取 bbox 并计算对齐位置
    top = (new_h - target_h) // 2  # 默认居中
...
def _composite_wide_top_strip_birefnet(canvas_path: str) -> None:
    """
    仅用于 3320×500（A5b）：
    1. 保留原始像素：BiRefNet 从未修改的原图中抠取 x=1032-2464、y=0-200 区域的前景；
    2. 铺整条 x=0-3320、y=0-40 #FFFFFF（规范顶部白条）；
    3. 将 BiRefNet RGBA 原位贴回；y=0-40 透明处透出白底，y=40-200 透明处仍可透出原背景；
    ...
    try:
        ...
        if strip_rgba is not None:
            draw = ImageDraw.Draw(img)
            # 整条 y=0-40 全宽铺白（已含抠图列，无需再单独铺列）
            draw.rectangle((0, 0, w, strip_h), fill=(255, 255, 255))
            # 贴回前景：透明处 y=0-40 透出白底
            img.paste(strip_rgba, (WIDE_STRIP_BIREFNET_X_MIN, mat_y_min), strip_rgba)
            img.save(canvas_path, "PNG")
            print(
                f"Step 5b / 3320×500 BiRefNet: 全条 y=0-{strip_h} 铺白, 抠图区 x={WIDE_STRIP_BIREFNET_X_MIN}-{WIDE_STRIP_BIREFNET_X_MAX} y={mat_y_min}-{mat_y_max} 前景贴回 → {canvas_path}",
                flush=True,
            )
            return
    except Exception as e:
...
def _safe_zone_scale_outpaint(
    ...
    *,
    ...
    if direct_to_canvas:
        ...
        # 仅 default (1976×464) 需要产出 tianchong.png 供 wide 消费
        tianchong_path = out_dir / "tianchong.png"
        if (width, height) == (1976, 464):
            print("  default: 先跑 A4 产出 tianchong.png 供 wide 消费...", flush=True)
            from safe_zone_scale_composite import composite_to_canvas_center
            fd, temp_canvas = tempfile.mkstemp(suffix=".png")
            os.close(fd)
            try:
                ...
                for r in range(4):
                    ...
                    try:
                        ...
                        print(f"  A4 图编失败，使用本地画布: {e}", file=sys.stderr)
                        import shutil
    ...
    if not skip_a4_outpaint:
        ...
        try:
            composite_to_canvas_center(
                ...
                center_x_ratio=0.5,  # A4：主体居中，左右延展均衡，避免左侧整条空白只填窗/光
                center_y_ratio=0.5,
            )
            max_fill_rounds = 4
            ...
            for r in range(max_fill_rounds):
                ...
                try:
                    ...
                    # Packy/Gemini 某些时段可能对图编模型 403 拒绝；为保证 auto 状态不断流程，
                    # 这里直接使用本地合成画布（含专有空白色 (0,0,1)）继续后续裁切/填充检测。
                    print(
