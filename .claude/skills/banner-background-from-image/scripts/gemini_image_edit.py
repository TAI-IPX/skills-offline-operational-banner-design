...
def _edit_image_micugemini(
    ...
    *,
    ...
    r = _req.post("https://www.micuapi.ai/v1/chat/completions", json=body, headers=h, timeout=600)
    r.raise_for_status()
    data = r.json()
    img_url, b64_out = "", ""
    ...
    if not img_url and not b64_out:
        raise RuntimeError("micugemini edit no image. resp: " + _json.dumps(data, ensure_ascii=False)[:500])
    out = _P(output_path)
...
def _edit_image_xingchengemini(
    ...
    *,
    ...
    """XingchenGemini 图编：通过 api.centos.hk /v1/chat/completions 编辑图片。
        ...
        if isinstance(ct, list):
            ...
            if m:
                ...
                if m:
                    img_url = m.group(1)
    if not img_url and not b64_out:
        raise RuntimeError("xingchengemini edit no image. resp: " + _json.dumps(data, ensure_ascii=False)[:500])
    out = _P(output_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    if img_url:
        dl = _req.get(img_url, timeout=300, proxies=None)
