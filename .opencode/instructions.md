...
- 所有入口脚本（`run_all_presets.py`、`run_changtu.py`、`run_battle_report.py` 等）在未指定图片路径时自动调用提取流程
...
**重要**：此推导使用当前 AI 模型（DeepSeek）在对话内完成，不调用任何外部 API。只有最后执行命令时才会由脚本调用生图 API（如 Packy7s Gemini 等）。
