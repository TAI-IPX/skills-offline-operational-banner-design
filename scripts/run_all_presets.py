...
def main():
    ...
    if getattr(args, "moxingemini", False) and not getattr(args, "packy7s", False) and not getattr(args, "moxingpt", False):
        ...
        env.setdefault("GEMINI_MODEL", env.get("MOXINGEMINI_MODEL_POOL", env.get("MOXINGEMINI_MODEL", "[次]gemini-3.1-flash-image-preview,[次]gemini-3-pro-image-preview")))
    ...
    # Step 1b: 3320×500 — 复用 Step 1a 的 tianchong.png（或 step1 回退），仅做 cover 缩放 + 顶部条带（不再跑第二次填充）
    need_wide = presets_to_run is None or "wide" in presets_to_run

    # Step 1b 前置：wide 需要 bbox 做智能对齐，始终在填充源图上重新检测
    # 写入独立文件 tianchong_bbox.txt，不覆盖 shared_subject_bbox.txt（后者来自 bg.png）
    if need_wide:
    ...
    if need_wide:
        if tianchong_path.is_file():
            ...
            print("Step 1b: 复用填充图 → 3320×500（cover + 顶部条带）...", flush=True)
        elif step1_path is not None and step1_path.is_file():
            _fill_source = step1_path
            print("Step 1b: (无 tianchong，用 step1 图回退) → 3320×500（cover + 顶部条带）...", flush=True)

    if _fill_source is not None:
        # 优先用 tianchong_bbox.txt（在填充源上独立检测），回退 shared_subject_bbox.txt
