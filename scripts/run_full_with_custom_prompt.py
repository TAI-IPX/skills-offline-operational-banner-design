...
def main() -> None:
    ...
    parser.add_argument("--moxingemini", "-moxingemini", action="store_true", dest="moxingemini", help="使用 MoxinGemini 专用 key 调用 Gemini（需 .env 中 MOXINGEMINI_API_KEY，端点 /v1/chat/completions）")
    ...
    # Step 2: -g 活动长图 → run_changtu.py，-g 战报 → run_battle_report.py，-g 排行榜 → run_ranking.py，-g 邮件长图 → run_email_poster.py
    ...
    if has_changtu:
        ...
        elif step1_bg_path and step1_bg_path.is_file():
            cmd_ct.extend(["--kv", str(step1_bg_path)])
    ...
    if has_email_poster:
        ...
        if getattr(args, "brand_logo", None):
            cmd_ep.extend(["--brand-logo", args.brand_logo])
