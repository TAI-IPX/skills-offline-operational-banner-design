"""
moxingpt_images_api.py — 标准 OpenAI Images API 封装（moxin.studio）
 ...
 用法：
   from moxingpt_images_api import generate_image, edit_image, chat_completions_image
 ...
 CLI:
   py scripts/moxingpt_images_api.py generate "A cat" -o output/cat.png
   py scripts/moxingpt_images_api.py edit input/photo.png "Remove bg" -o output/edited.png
"""
...
BASE_URL = os.environ.get("MOXINGPT_BASE_URL", "https://www.moxin.studio/v1").rstrip("/")
DEFAULT_MODEL = "[次]gpt-image-2"
...
def _get_api_key() -> str:
    ...
    if not key or not key.startswith("sk-"):
        raise RuntimeError(
            "MOXINGPT_API_KEY 未设置或格式错误（需 sk- 开头）。"
            "请在 .env 或环境变量中配置。"
        )
    return key
...
def _get_proxies() -> dict | None:
    ...
    需要走代理时显式设置 MOXINGPT_USE_PROXY=1 才会探测 HTTPS_PROXY / Windows 注册表代理。
    MOXINGPT_NO_PROXY=1 仍受支持（向后兼容，效果与不设置任何代理开关一致）。
    """
    no_proxy = os.environ.get("MOXINGPT_NO_PROXY", "").strip()
    if no_proxy.lower() in ("1", "true", "yes"):
        return None

    use_proxy = os.environ.get("MOXINGPT_USE_PROXY", "").strip()
...
def generate_image(
    ...
    *,
    ...
    max_retries: 下载失败时重试次数（CDN 不可达时重新调用 API）
    """
    import requests as _requests
    api_key = _get_api_key()
    model = model or os.environ.get("MOXINGPT_MODEL", DEFAULT_MODEL).strip()
    _size = size or os.environ.get("MOXINGPT_SIZE", "auto").strip()
    _quality = quality or os.environ.get("MOXINGPT_QUALITY", "auto").strip()
    ...
    for attempt in range(max_retries):
        ...
        print(f"[moxingpt t2i] 生成图片（{model}，size={_size}，quality={_quality}）...", flush=True)
        ...
        if not img_url and not b64_out:
            print(f"[moxingpt t2i] 未获取到图片。响应: {json.dumps(data, ensure_ascii=False)[:500]}", file=sys.stderr)
            return None

        try:
            img_bytes = _download_image(img_url, b64_out)
        except Exception as e:
            if attempt < max_retries - 1 and "oss.filenest.top" in str(e):
                print(f"[moxingpt t2i] CDN 不可达，重试 ({attempt + 2}/{max_retries})...", flush=True)
                time.sleep(1)
                continue
            raise
        ...
        if output_path:
            ...
            print(f"[moxingpt t2i] 已保存: {out_path}", flush=True)
...
def edit_image(
    ...
    *,
    ...
    print(f"[moxingpt edit] 编辑图片（{model}）...", flush=True)
    resp = _requests.post(
        url,
        files=files,
        data=data_fields,
    ...
    img_url, b64_out = _resolve_image_url(data)
    if not img_url and not b64_out:
        print(f"[moxingpt edit] 未获取到图片。响应: {json.dumps(data, ensure_ascii=False)[:500]}", file=sys.stderr)
        return None
    ...
    if output_path:
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_bytes(img_bytes)
        print(f"[moxingpt edit] 已保存: {out_path}", flush=True)
        return out_path.resolve()
    return img_bytes
...
def chat_completions_image(
    ...
    *,
    ...
    for ip in image_paths:
        p = Path(ip)
        if not p.is_file():
            print(f"[moxingpt chat] 图片不存在: {p}", file=sys.stderr)
            continue
    ...
    print(f"[moxingpt chat] 生成图片（{model}）...", flush=True)
    resp = _requests.post(url, json=body, headers=_get_headers(), timeout=600, proxies=_get_proxies())
    data = resp.json()

    img_url, b64_out = _resolve_image_url(data)
    if not img_url and not b64_out:
        print(f"[moxingpt chat] 未获取到图片。响应: {json.dumps(data, ensure_ascii=False)[:500]}", file=sys.stderr)
        return None
    ...
    if output_path:
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_bytes(img_bytes)
        print(f"[moxingpt chat] 已保存: {out_path}", flush=True)
        return out_path.resolve()
    return img_bytes


def main():
    parser = argparse.ArgumentParser(description="MoxinGPT Images API CLI")
