    ...
    if getattr(args, "moxingemini", False):
        ...
        if moxingemini_key and moxingemini_key.strip().startswith("sk-"):
            ...
            if not os.environ.get("GEMINI_MODEL"):
                os.environ["GEMINI_MODEL"] = os.environ.get("MOXINGEMINI_MODEL_POOL", os.environ.get("MOXINGEMINI_MODEL", "[次]gemini-3.1-flash-image-preview,[次]gemini-3-pro-image-preview"))
        else:
