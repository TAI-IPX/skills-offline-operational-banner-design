...
def call_micu_vision_with_images(
    ...
    *,
    ...
    if not key:
        print("[Vision/debug] MICU_API_KEY is empty, aborting", flush=True)
        return None
    ...
    print(f"[Vision/debug] MICU POST {url} model={micu_vision_models()[0] if micu_vision_models() else 'N/A'}", flush=True)
