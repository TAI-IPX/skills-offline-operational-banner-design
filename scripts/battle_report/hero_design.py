...
def _gemini_vision_with_images(
    ...
    if not key:
        print("[Vision/debug] GEMINI_API_KEY not set, aborting", flush=True)
        return None
    print(f"[Vision/debug] GEMINI_API_KEY={key[:12]}... base_url={os.environ.get('GOOGLE_GEMINI_BASE_URL','')[:40]}", flush=True)
    ...
    for p in image_paths:
        ...
        if block:
            parts.append(block)
    if len(parts) <= 1:
        print(f"[Vision/debug] No valid image blocks (total_parts={len(parts)}), aborting", flush=True)
        return None
    print(f"[Vision/debug] {len(parts)-1} image block(s) encoded, calling _post_vision_parts", flush=True)
...
def call_vision_with_images(prompt: str, image_paths: list[Path], *, thumb_max: int = 0) -> str | None:
    ...
    if vision_backend() == "micu":
        print("[Vision/debug] using MICU Vision path", flush=True)
        ...
        return None
    _last_vision_source = "gemini"
    print("[Vision/debug] using Gemini Vision path", flush=True)
    return _gemini_vision_with_images(prompt, image_paths, thumb_max=thumb_max)
...
def _post_vision_parts(parts: list[dict], *, timeout: int = 45) -> str | None:
    ...
    for key in api_keys:
        ...
        for model in _vision_models():
            ...
            print(f"[Vision/debug] POST {url} key_prefix={key[:8]}... auth={'Bearer' if key.startswith('sk-') else 'query'}", flush=True)
            for attempt in range(2):
                try:
                    with urllib.request.urlopen(req, timeout=timeout) as r:
                        data = json.loads(r.read())
                    print(f"[Vision/debug]  response status={r.status} content_len={len(json.dumps(data))}", flush=True)
                    for part in data.get("candidates", [{}])[0].get("content", {}).get("parts", []):
                        if "text" in part:
                            return part["text"]
                except urllib.error.HTTPError as e:
                    print(f"[Vision/debug] HTTPError {e.code}: {e.reason} (attempt {attempt+1})", flush=True)
                    if e.code == 401:
                        if key != api_keys[-1]:
                            try_next_key = True
                    ...
                    print(f"[Vision/debug] Exception: {type(e2).__name__}: {e2} (attempt {attempt+1})", flush=True)
                    if attempt == 0:
                        time.sleep(2)
            if try_next_key:
                break
