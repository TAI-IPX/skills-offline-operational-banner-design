...
def decor_image_backend() -> str:
    ...
    if img_be in ("xingchengpt", "packygpt", "micugpt2", "xinchengpt", "moxingpt"):
        return img_be
...
def _run_decor_i2i(
    ...
    *,
    ...
    if backend in ("xingchengpt", "packygpt", "micugpt2", "xinchengpt", "moxingpt"):
...

{
  ...
  "depth_style": "选择最匹配的一项: shallow_bokeh / deep_atmospheric / flat_graphic / layered_parallax"
    ...
    print(f"[Vision/debug] call_vision_with_images returned: {'OK' if text else 'None'} ({len(text or '')} chars)", flush=True)
    style_info: dict = {}
    if text:
