...
def _get_image_config(args=None) -> dict:
    # 按命令行标志优先级选择 API Key 和 Base URL
    if args is not None and getattr(args, "moxingpt", False):
        api_key = get_env_key("MOXINGPT_API_KEY") or ""
        api_base = (os.environ.get("MOXINGPT_BASE_URL", "").strip() or "https://www.moxin.studio") + "/v1"
        model = os.environ.get("MOXINGPT_MODEL", "gpt-image-2")
        size = os.environ.get("MOXINGPT_SIZE", "auto")
        quality = os.environ.get("MOXINGPT_QUALITY", "auto")
    elif args is not None and getattr(args, "packygpt", False):
        api_key = get_env_key("PACKYGPT_API_KEY") or ""
        api_base = "https://www.packyapi.com/v1"
        model = "gpt-image-2"
        size = "auto"
        quality = "auto"
    elif args is not None and getattr(args, "micugpt2", False):
        api_key = get_env_key("MICUAPI_API_KEY") or ""
        api_base = "https://api.micuapi.ai/v1"
        model = "gpt-image-2"
        size = "auto"
        quality = "auto"
    elif args is not None and getattr(args, "xingchengpt", False):
        api_key = get_env_key("XINGCHENGGPT_API_KEY") or ""
        api_base = (os.environ.get("XINGCHENGGPT_BASE_URL", "").strip() or "https://api.newapi.pro") + "/v1"
        model = os.environ.get("XINGCHENGGPT_MODEL", "gpt-image-2")
        size = os.environ.get("XINGCHENGGPT_SIZE", "auto")
        quality = os.environ.get("XINGCHENGGPT_QUALITY", "auto")
    elif args is not None and getattr(args, "xinchengpt", False):
        api_key = get_env_key("XINCHENGPT_API_KEY") or ""
        api_base = (os.environ.get("XINCHENGPT_BASE_URL", "").strip() or "https://api.centos.hk") + "/v1"
        model = os.environ.get("XINCHENGPT_MODEL", "gpt-image-2")
        size = os.environ.get("XINCHENGPT_SIZE", "auto")
        quality = os.environ.get("XINCHENGPT_QUALITY", "auto")
    else:
        # 兜底：按环境变量顺序自动选择
        api_key = (get_env_key("MOXINGPT_API_KEY")
                   or get_env_key("XINCHENGPT_API_KEY")
                   or get_env_key("XINGCHENGGPT_API_KEY")
                   or get_env_key("PACKYGPT_API_KEY")
                   or "")
