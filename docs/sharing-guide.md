...
| 多种 AI 后端 | 支持 6 种图像生成/编辑后端（Gemini、gpt-image-2、即梦 等） |
...
```
          ...
          ┌───────────────────┼───────────────────┐
          ▼                   ▼                   ▼
      Gemini API        gpt-image-2          即梦
    (生图+编辑+Vision)   (packygpt/micugpt2)  (jimeng/t8star)
```
...
| 文件 | 功能 |
|------|------|
| `scripts/generate_from_description.py` | **核心**：从描述文字生成 Banner 背景图。支持 nano-banana/gemini/t8star/packygpt/micugpt2/jimeng 等后端，含 prompt 优化器 |
